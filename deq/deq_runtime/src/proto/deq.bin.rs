...
pub struct CheckModel {
    ...
    /// Normal window projection removes that vertex, not the error.
    ...
    /// Interior and reconvergent paths require the full model. The same window
    /// projection then retains exactly the same local error slots and effects.
