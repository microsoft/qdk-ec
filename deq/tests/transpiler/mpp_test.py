...
@pytest.mark.skipif(not _has_errata, reason="errata package not installed")
class TestMppErrataLocations:
    """Test MPP conversion to errata Locations."""

    def test_mpp_produces_measurement_locations(self) -> None:
        inst = Instruction(
            name="MPP",
            targets=[
                PauliTarget("Z", 0),
        ...
        locs = instructions_to_locations([inst])
        from errata.circuits.clifford import Measurement

        measurements = [loc for loc in locs if isinstance(loc.operation, Measurement)]
        assert len(measurements) == 2

    def test_mpp_observable_is_multi_qubit(self) -> None:
        inst = Instruction(
            name="MPP",
            targets=[
                PauliTarget("Z", 0),
        ...
        # After decomposition, MPP Z0*Z1 is implemented via CX+M.
        # Verify that exactly one measurement is produced.
        measurements = [loc for loc in locs if isinstance(loc.operation, Measurement)]
