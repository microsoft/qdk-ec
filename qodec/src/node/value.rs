...
impl<'model> Value<'model> {
    ...
    fn field(&self, field: &str) -> Option<Self> {
        Some(match (self, field) {
            ...
            (Self::Instruction(value), "inputs") => Self::Sequence(Sequence::Operands(&value.inputs)),
            (Self::Instruction(value), "outputs") => Self::Sequence(Sequence::Operands(&value.outputs)),
            ...
            (Self::Gadget(value), "inputs") => Self::Sequence(Sequence::Encodings(&value.inputs)),
            (Self::Gadget(value), "outputs") => Self::Sequence(Sequence::Encodings(&value.outputs)),
