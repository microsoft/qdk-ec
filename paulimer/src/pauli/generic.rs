...
where
    T: PauliMutable + NeutralElement<NeutralElementType = T>,
{
    let mut pauli: T = <T as NeutralElement>::neutral_element_of_size(pauli_string_without_phase.len());
            ...
            match character {
                'X' | 'x' | 'Y' | 'y' | 'Z' | 'z' => {
                    if let Some(prev) = pauli_char {
                        accumulate_pauli(&mut pauli, prev, index, has_digits)?;
                    }
...
fn accumulate_pauli<T: PauliMutable>(
    pauli: &mut T,
    pauli_char: char,
    index: usize,
    has_digits: bool,
) -> Result<(), PauliStringParsingError> {
    if !has_digits {
        return Err(PauliStringParsingError);
    }
    match pauli_char {
